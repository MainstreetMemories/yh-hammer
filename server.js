...
app.post('/api/upload-json', async (req, res) => {
  try {
    ...
    const extracted = {
      ...
      phone: field('Phone') || '',
      email: (text.match(/Email:\s*([\w\.\-]+@[\w\.\-]+\.\w+)/i) || ['',''])[1] || text.match(/\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/)?.[0] || '',
...
app.post('/api/save-confirmed', async (req, res) => {
  try {
    ...
    // Homeowner payment - only log if there's a value AND it changed
